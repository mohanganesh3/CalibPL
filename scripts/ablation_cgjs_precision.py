"""
Ablation: Pseudo-Label Precision of CGJS vs. Baseline (500-IMAGE STATISTICAL RIGOR).
"""
...
# COCO 80 categories (YOLO index) to COCO Category IDs (1-indexed with gaps)
...
def run_precision_ablation():
    ...
    rules = [
        ...
        {'name': 'CalibPL + CGJS (Rel=0.6, S=0.8)', 'type': 'calib_cgjs', 'rel': 0.6, 'stab': 0.8}
    ]
    
    # Scale to 500 images for statistical significance
    test_images = list(img_name_to_anns.keys())[:500]
    ...
    print(f"Running Precision Ablation on {len(test_images)} images...")
    start_time = time.time()
    
    for i, img_name in enumerate(test_images):
        if i % 50 == 0 and i > 0:
            elapsed = time.time() - start_time
            print(f"  Processed {i}/{len(test_images)} images... ({elapsed:.1f}s)")
        ...
        # Precompute CGJS for the image once
    ...
    print("\n--- FINAL RESULTS (500 IMAGES) ---")
    for name, stats in results.items():
        precision = stats['tp'] / max(1, stats['tp'] + stats['fp'])
        print(f"{name}: Precision = {precision:.4f} (TP={stats['tp']}, FP={stats['fp']})")
